...
- **Fonts** - IBM Plex Sans Condensed (Google Fonts, titles/headings), IBM Plex Serif (Google Fonts, body/UI), and self-hosted MesloLGM Nerd Font Mono Regular (inline code, code blocks, and `<pre>`). These are the only three typefaces used by the app.
